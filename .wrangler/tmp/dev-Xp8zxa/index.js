...
// .wrangler/tmp/bundle-vzwpmT/checked-fetch.js
...
// .wrangler/tmp/bundle-vzwpmT/strip-cf-connecting-ip-header.js
...
authRouter.patch("/me", authMiddleware, async (c) => {
  ...
  if (session_duration_minutes !== void 0) {
    if (![30, 45, 50, 60].includes(session_duration_minutes)) {
      return c.json(
        { success: false, error: "La duraci\xF3n debe ser 30, 45, 50 o 60 minutos" },
        400
      );
    }
...
async function generateSlotsForDate(db, date, psychologistId) {
  ...
  const psych = await db.prepare("SELECT session_duration_minutes FROM psychologists WHERE id = ?").bind(psychologistId).first();
...
slotsRouter.get("/", async (c) => {
  ...
  const psych = await c.env.DB.prepare("SELECT id, session_duration_minutes FROM psychologists LIMIT 1").first();
  if (!psych) {
    return c.json({ success: true, data: [] });
  }
  ...
  const availableSlots = existingSlotsResult.results.filter((s) => s.available === 1).sort((a, b) => a.start_time.localeCompare(b.start_time));
  return c.json({
    success: true,
...
slotsRouter.post("/", authMiddleware, async (c) => {
  ...
  const config = await c.env.DB.prepare("SELECT session_duration_minutes FROM psychologists WHERE id = ?").bind(psychologistId).first();
...
bookingsRouter.post("/", async (c) => {
  ...
  if (!isPsychologist) {
    ...
    ).bind(patient_email, slot.date, slot.start_time, slot.end_time).first();
    if (overlap) {
...
bookingsRouter.post("/search", async (c) => {
  ...
  if (!email || !phone) {
    return c.json({ success: false, error: "Email y tel\xE9fono requeridos" }, 400);
  }
  const result = await c.env.DB.prepare(
...
bookingsRouter.patch("/:id", async (c) => {
  ...
  if (!email || !phone || !new_slot_id) {
    return c.json({ success: false, error: "Email, tel\xE9fono y nuevo turno son requeridos" }, 400);
  }
  ...
  const conflict = await c.env.DB.prepare(
    `SELECT b.id FROM bookings b
     ...
     AND NOT (s.end_time <= ? OR s.start_time >= ?)`
  ).bind(email, newSlot.date, id, newSlot.start_time, newSlot.end_time).first();
  if (conflict) {
    return c.json({ success: false, error: "Ya ten\xE9s una reserva en ese horario" }, 409);
  }
  try {
...
bookingsRouter.delete("/:id", async (c) => {
  ...
  const { email, phone } = body;
  if (!email || !phone) {
    return c.json({ success: false, error: "Email y tel\xE9fono requeridos para cancelar" }, 400);
  }
      ...
      try {
        const slotResult = await db.prepare(
          `INSERT INTO slots (psychologist_id, date, start_time, end_time, recurring_booking_id)
             VALUES (?, ?, ?, ?, ?)`
        ).bind(psychologistId, current, time, end_time, recurringId).run();
        ...
        await db.prepare(
          `INSERT INTO bookings (slot_id, patient_name, patient_email, patient_phone, recurring_booking_id)
             VALUES (?, ?, ?, ?, ?)`
        ).bind(slotId, patientName, patientEmail, patientPhone, recurringId).run();
        await db.prepare("UPDATE slots SET available = 0 WHERE id = ?").bind(slotId).run();
        created++;
      } catch {
        skipped++;
      }
...
recurringRouter.delete("/:id", async (c) => {
  ...
  if (!isPsychologist) {
    ...
    if (!email || !phone) {
      return c.json({ success: false, error: "Email y tel\xE9fono requeridos para cancelar la recurrencia" }, 400);
    }
  }
  ...
  const recurring = await c.env.DB.prepare(query).bind(...params).first();
  if (!recurring) {
  ...
  if (slotIds.length > 0) {
    ...
    for (let i = 0; i < slotIds.length; i += batchSize) {
      ...
      await c.env.DB.prepare(`DELETE FROM bookings WHERE slot_id IN (${placeholders})`).bind(...chunk).run();
...
recurringRouter.patch("/:id/reschedule-from", async (c) => {
  ...
  if (!email || !phone || !from_date || !new_time) {
    return c.json({ success: false, error: "Email, tel\xE9fono, fecha de inicio y nueva hora son requeridos" }, 400);
  }
  ...
  ).bind(id, email, phone).first();
  if (!recurring) {
    return c.json({ success: false, error: "Recurrencia no encontrada o datos incorrectos" }, 404);
  }
  const config = await c.env.DB.prepare("SELECT session_duration_minutes FROM psychologists WHERE id = ?").bind(recurring.psychologist_id).first();
  const sessionDuration = config?.session_duration_minutes ?? 45;
  ...
  for (const slot of futureSlots.results) {
    const conflict = await c.env.DB.prepare(
      `SELECT id FROM slots
       ...
       AND NOT (end_time <= ? OR start_time >= ?)`
    ).bind(recurring.psychologist_id, slot.date, id, new_time, newEndTime).first();
    if (conflict) {
      continue;
    }
...
// .wrangler/tmp/bundle-vzwpmT/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-vzwpmT/middleware-loader.entry.ts
