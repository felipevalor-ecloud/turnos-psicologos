...
// .wrangler/tmp/bundle-Ws8h1X/checked-fetch.js
...
// .wrangler/tmp/bundle-Ws8h1X/strip-cf-connecting-ip-header.js
...
bookingsRouter.delete("/:id", async (c) => {
  ...
  const appointmentDate = /* @__PURE__ */ new Date(`${booking.date}T${booking.start_time}:00Z`);
  const hoursUntil = (appointmentDate.getTime() - Date.now()) / (1e3 * 3600);
  if (hoursUntil < 24) {
    return c.json(
      {
        success: false,
        error: "Las cancelaciones deben realizarse con al menos 24 horas de anticipaci\xF3n"
      },
      400
    );
  }
...
// .wrangler/tmp/bundle-Ws8h1X/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Ws8h1X/middleware-loader.entry.ts
